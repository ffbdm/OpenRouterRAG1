...
## Internal Movement
- Describe how modules within `AGENTS.md`, `attached_assets`, `client`, `components.json`, `design_guidelines.md`, `drizzle.config.ts`, `package-lock.json`, `package.json`, `plan-dynamicToolCalls.prompt.md`, `plans`, `postcss.config.js`, `scripts`, `server`, `shared`, `tailwind.config.ts`, `tsconfig.json`, `vercel.json`, `vite.config.ts` collaborate (queues, events, RPC calls, shared databases).

...
> oje você usa pdf-parse (PDFParse.getText()), que é ótimo pra “texto corrido”, mas não preserva layout/posições — por isso tabela vira texto “quebrado”. Pra melhorar extração de tabelas, as opções mais comuns são:PDF.js (pdfjs-dist) + heurísticas de layout (Node/TS): pega itens de texto com coordenadas (x/y), agrupa por linha (y) e coluna (clusters de x) e reconstrói em Markdown/CSV. Melhor custo/controle quando o PDF tem texto selecionável.
